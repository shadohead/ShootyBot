...
async def on_message(message):
    ...
    if message.author == bot.user and message.content.startswith(DEFAULT_MSG):
        ...
        shooty_context = get_shooty_context_from_channel_id(
            channel_id, shooty_context_dict
        )
...
@bot.hybrid_command(
    name="st", description="Starts a Fresh Shooty Session (FSS™)", with_app_command=True
)
...
def backup_user_sets(channel_id, old_shooty_context: ShootyContext):
    ...
    backup_shooty_context.bot_ready_user_set = set(
        old_shooty_context.bot_ready_user_set
    )
    backup_shooty_context.bot_fullstack_user_set = set(
        old_shooty_context.bot_fullstack_user_set
    )
    backup_shooty_context.bot_soloq_user_set = set(
        old_shooty_context.bot_soloq_user_set
    )
...
async def cmd_mention_session(ctx):
    ...
    if (
        not shooty_context.bot_soloq_user_set
        and not shooty_context.bot_fullstack_user_set
    ):
    ...
    mention_message = "".join(
        user.mention + " "
        for user in shooty_context.bot_soloq_user_set.union(
            shooty_context.bot_fullstack_user_set
        )
        if not user.bot
    )
...
async def cmd_clear_session(ctx):
    ...
    logging.info(
        ...
        + str(
            to_names_list(
                shooty_context.bot_soloq_user_set.union(
                    shooty_context.bot_fullstack_user_set
                )
            )
        )
...
async def cmd_scheduled_session(ctx, game_time):
    ...
    try:
        ...
        if seconds_to_wait < 0:
            ...
            await ctx.send(
                "Shooty session can only be scheduled up to 4 hrs in advance."
            )
            return
...
async def cmd_cancel_scheduled_session(ctx):
    ...
    # timer.cancel()
    await ctx.send(
        f"Canceled scheduled session - {scheduled_time.strftime('%H:%M %p')}"
    )
...
async def cmd_set_role_code(ctx, role_code):
    ...
    shooty_context.set_channel_data(
        role_code=shooty_context.role_code,
...
async def cmd_lfg(ctx):
    ...
    if shooty_context.game_name is None:
        await ctx.send(
            f"Set this channel's game name to see other queues with the same name ```$stsg <game name>```"
        )
        return
...
async def cmd_beacon(ctx, message):
    ...
    for context in shooty_context_dict.values():
        if (
            context.game_name == shooty_context.game_name
            and context.channel is not shooty_context.channel
        ):
...
@bot.hybrid_command(
    name="shootysetgame", description="Set the game name for LFG features"
)
...
async def on_command_error(ctx, error):
    if (
        ctx.message.content.startswith("$shooty")
        or ctx.message.content.startswith("$st")
        and isinstance(error, discord.ext.commands.errors.CommandNotFound)
    ):
...
async def on_reaction_add(reaction, user):
    ...
    shooty_context = get_shooty_context_from_channel_id(channel_id, shooty_context_dict)
    logging.info(
        f"{id(shooty_context)} channel: {str(shooty_context.channel)}, soloq: {id(shooty_context.bot_soloq_user_set)}, fullstack: {id(shooty_context.bot_fullstack_user_set)}"
    )
    ...
    # handle full stack only players
        ...
        if not shooty_context.is_soloq_user(user):
            ...
            logging.info(
                "fullstack:" + str(to_names_list(shooty_context.bot_fullstack_user_set))
            )
        ...
        logging.info(
            "ready_set:" + str(to_names_list(shooty_context.bot_ready_user_set))
        )
    ...
    elif reaction.emoji == "\N{LOUDSPEAKER}":
...
def get_shooty_context_from_channel_id(
    channel_id, shooty_context_dict: dict[str, ShootyContext]
) -> ShootyContext:
